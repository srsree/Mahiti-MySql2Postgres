    ...
    def test_truncate(self):
        ...
        if reset_seq:
            self.assertRegex(squeeze(reset_seq),
                                 r"^SELECT pg_catalog.setval\('%s', \d+, true\);$" % self.table1.name)
...
#                                        '^CREATE TABLE "%s" \(.*\) WITHOUT OIDS;$' % self.table1.name)
        ...
        if seq_cmds:
            ...
            self.assertRegex(squeeze(seq_cmds[0]),
                                     r'^DROP SEQUENCE IF EXISTS %s_([^\s]+)_seq CASCADE;$' % self.table1.name)
            self.assertRegex(squeeze(seq_cmds[1]),
                                     r'^CREATE SEQUENCE %s_([^\s]+)_seq INCREMENT BY 1 NO MAXVALUE NO MINVALUE CACHE 1;$' % self.table1.name)
            self.assertRegex(squeeze(seq_cmds[2]),
                                     r"^SELECT pg_catalog.setval\('%s_([^\s]+)_seq', \d+, true\);$" % self.table1.name)
